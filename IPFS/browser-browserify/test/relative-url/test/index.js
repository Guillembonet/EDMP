...
tape('same path works on dev and deployed', function (t) {
  ...
  t.equal(
    rurl('/', { protocol: 'http', host: 'localhost:8888'}, map, def),
    'ws://localhost:8888/'
  )
  t.equal(
...
tape('universal url still works', function (t) {
  var location = { protocol: 'http', host: 'localhost:8888' }
  t.equal(
...
tape('protocol defaults to not change', function (t) {
  ...
  t.equal(
    rurl('/', { protocol: 'http', host: 'localhost:8888'}),
    'http://localhost:8888/'
  )
  t.equal(
...
tape('query string, if you want that!', function (t) {
  ...
  t.equal(
    ...
    'ws://localhost:8888/foo/bar?whatever=true'
  )
...
tape("don't mess with url that starts out absolute", function (t) {
  ...
  t.equal(
    rurl('ws://localhost:8888/', { protocol: 'http', host: 'localhost:8888'}, map, def),
    'ws://localhost:8888/'
  )
  t.equal(
    rurl('ws://localhost:8888/', { protocol: 'https', host: 'server.com'}, map, def),
    'ws://localhost:8888/'
  )
